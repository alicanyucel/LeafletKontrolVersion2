...
import { Map, Control, DomUtil, ZoomAnimEvent , MapOptions, LoadingOptions } from 'leaflet';
...
export class OsmMapComponent implements OnInit, OnDestroy {
  ...
  // Use a compact attribution control for small map container widths
    ...
    Control.Attribution.prototype.addTo = function(map) {
      ...
      const parent     = this.getContainer.parentNode;
      ...
      if (map._container.offsetWidth <= 600) {
        DomUtil.addClass(this._getContainer, 'leaflet-compact-attribution');
      }
      // update on map resize111111111
      map.on('resize', () => {
        if (map._container.offsetWidth > 600) {
          DomUtil.removeClass(this._getContainer, 'leaflet-compact-attribution');
        } else {
